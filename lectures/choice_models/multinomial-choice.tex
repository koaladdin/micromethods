\documentclass[dvipsnames,table,mathserif,aspectratio=169]{beamer}

\usetheme{default}
\usepackage{luatex85}  % Add support for legacy pdfTeX commands

\usepackage[english]{babel}
\usepackage{fontspec}  % Modern font handling for LuaTeX
...
%% Uncomment to change background color
...
\begin{document}
\title{Binary/Multinomial Choice and\\ Statistical Demand Models*}
\author{Charlie Murry}
\institute{Empirical Methods | University of Michigan}
\date{\today}


\frame{\titlepage \footnotesize \emph{*Based heavily on notes from Chris Conlon (NYU Stern), Rich Sweeney (BC) and others.}}


\section{Multinomial Choice}
\begin{frame}
...
\item Choosing a mutual fund/manager.
\item Choosing where to live.
\item Choosing a brand/model of (yogurt, laundry detergent, orange juice, cars, etc.).
 \end{itemize}
\end{frame}
...
\end{eqnarray*}
\emph{*Notice additive separability.*}\\[1em]
\pause
...
\begin{frame}
\frametitle{Nonparametric Setup}
In order to compute choice probabilities, we must compute a $J$ dimensional integral: $f(\varepsilon_i)$.
\begin{eqnarray*}
...
\item e.g. we only observe if $U_{ij} > U_{ik}$, not by how much.
\item Logit pins down $f(\varepsilon_i)$ sufficiently with parametric restrictions (no covariance terms).
\item Probit does not. We must generally normalize one dimension of $f(\varepsilon_i)$ in the probit model. Usually a diagonal term of $\Omega$ so that $\omega_{11} =1$ for example.
\end{wideitemize}
\end{frame}
...
\item Lewbel (2000) does random utility with additively separable but nonparametric error.\item Berry and Haile (2024, ECMA) with non-separable error (and endogeneity).
\end{wideitemize}
\end{wideitemize}
\end{frame}
...
\begin{frame}
\frametitle{Multinomial Logit (MNL)}
\begin{itemize}
...
E[\max_j U_{ij}] = \log \left(\sum_j \exp[V_{ij}] \right) + C
\end{eqnarray*}
\item Expected utility of best option (no knowledge of realized $\varepsilon_i$) does not depend on $\epsilon_{ij}$.
...
\item This is on top of the scale normalization. But these don't impact the behavioral implications of the model. 
\end{itemize}
\end{frame}
...
\item In fact, if price is a regressor, we can scale other parameters by price coefficient to get utils in terms of \$.
\end{itemize}
\end{frame}
...
\item Sometimes we have \alert{choice based samples} where we oversample people who choose a particular alternative. Manski and Lerman (1977) show we can get consistent estimates for all but the constant. This requires knowledge of the difference between the true rate $A_j$ and the choice-based sample rate $S_j$.
...
\item This is not a desireable property for most empirical work. 
\end{itemize}
\end{frame}
...
This ends up being many terms if there are many people and many choices in the dataset. But it's not really a problem for modern computers. Plus, the problem is convex. 
% \begin{itemize}
...
\item Helpful to write $u_{ij} = x_j \beta -\alpha p_j + \epsilon_{ij}$ (assumes aggregation!).
\begin{eqnarray*}
...
\item The price elasticity is increasing in own price! (no income effects...)
...
\item The othr hard part is figuring out what identifies $\lambda$
\end{itemize}
\end{frame}
...
\end{eqnarray*}
 ...
 \item In practice these are not that different from linear \alert{random effects models} you may have learned about. [random effects]
...
\item Observed heterogeneity does not make life difficult so the more of that the better!
\end{itemize}
\end{frame}
...
\item But if you have 1000 products and two random coefficients, you are asking for a lot.
 \end{itemize}
\end{frame}
...
\end{eqnarray*}
\item When we are doing IIA logit, this problem is globally convex and is easy to estimate using BHHH or another quasi-Newton Method.
...
\begin{frame}
  \frametitle{Demand and Supply in Market Equilibrium}
  
  \begin{itemize}
  \item Goals:
    \begin{enumerate}
    \item Estimating discrete choice demand models with aggregate data
    \item Solutions of endogeneous prices $\rightarrow$ Positive bias in OLS
      price coefficients
    % \item Build in cost side and imperfect competition
    \item Usefulness of ``micro'' data on consumer choices like typical MNLogit (micro BLP and Petrin)
    \end{enumerate} \bigskip
  \item Applications:
    \begin{enumerate}
    \item Demand for cars (BLP, micro BLP, Petrin)
    \item Welfare effects of introducing the minivan (Petrin)
    \item Market power in ready to eat cereal industry (Nevo)
    \end{enumerate}
  \end{itemize}
  \end{frame}
  
  
  \begin{frame}{Solving Some Problems}
    We saw from MNLogit that the choice probabilities were
    \begin{eqnarray*}
      s_{ij} &=&  \frac{\exp[x_{j} \beta_i  ]}{1+\sum_k \exp[x_{k} \beta_i  ]} \\
      %  &=& \sum_{s=1}^{ns} w_s \frac{\exp[x_{j} (\overline{\beta} + \Sigma \nu_{is})  ]}{1+\sum_k \exp[x_{k} (\overline{\beta} + \Sigma \nu_{is})  ]} 
      \end{eqnarray*}
  
But if we only have aggregate data, we need to sum over the $i$'s to get aggregate shares. 
\begin{align*}
s_{j} = \int \frac{\exp[x_{j} \beta_i  ]}{1+\sum_k \exp[x_{k} \beta_i  ]} f(\beta_i | \theta) \\
\end{align*}

Also, one if the $x$'s is endogenous?
  \end{frame}


\begin{frame}{Endogeneity}
  
  Since we don't  have individual-level data anyway, we can get a bit create.\\[2em]

  Let's start backwards with GMM IV: $E[\xi | Z] = 0$.\\[2em]

  How can we take advatage of this general framework for dealing with endogeneity?

  Rewrite the choice problem as:
  $$u_{ij} = \delta_j + \mu_{ij})(\theta) + \epsilon_{ij} $$

  where $$\delta_j = x_j \beta + \alpha p_j + \xi_j$$.
and where we broke out $p$ from the vector $x$ to denote it is special (endogenous).\\[2em]

The $\delta$ equation is linear in the endogenous variable, so if $delta$ were data, we could use the GMMIV (or 2SLS) framework. 

\end{frame}

\begin{frame}{Finding $\delta$}

  Berry (1994) shows that this general demand structure, with extreme value $\epsilon$ implies a unique vector of $delta$ that rationalizes 

  $$s_{j}^{\text{data}} = s_j^{\text{model}}(\theta)$$

  (Later work generalizes this to any demand system that satisfies very mild properties)\\[2em]

  Therefore, we use the shares condition to solve for a unique $\delta$, and then treat that $\delta$ as data and do some form of linear IV regression.\\[2em] 

  Problem: the $s_{j}^{\text{data}} = s_j^{\text{model}}(\theta)$ depends on additional parameters $\theta$ that define $\mu_{ij}(\theta)$.\\[2em]

  So we need to estimate these as well!
\end{frame}

\begin{frame}{Procedure}
  % \framesubtitle{concrete example}

\begin{itemize}
  \item[1.] Guess $\theta$.
  \item[2.] Compute aggregate shares a la MNLogit. 
  $$ s_{j} = \int \frac{\exp[\delta_{j} + \mu_{ij}(\theta)]}{1+\sum_k \exp[\delta_{k} + \mu_{ik}(\theta) ]} d\mu_{i} $$
  \item[3.] Solve the NL system of equations and unknowns 
  $$s_j = s_j^\text{data}$$
  \item[4.] Form GMM objective function\footnote{Note: we can profile out $\beta$ and estimate in a second step.}
...
\begin{frame}{Some Unresolved Issues}

  Notice that we have to estimate $\theta$ and $\beta$ and $\alpha$. Choice of IVs:
  \begin{itemize}\setlength{\itemsep}{1em}
    \item $X$ for $X$ (to identify $\beta$) 
    \item cost shifter(?) for $p$ (to identify $\alpha$)
    \item What for $\theta$?
  \end{itemize}
  
  Should we really be forcing the shares to match? This is like putting infinite weight on one particular moment.\\[2em]

  These integrals can get nasty. How much computation? Numerical error?\\[2em]
