\documentclass[11pt,dvipsnames,table,aspectratio=169]{beamer}
% \usepackage{fontspec}
% \setmainfont{Latin Modern Roman}  % or another Unicode font
% \usepackage{fontspec}
...
% Add after natbib but before tikz packages
\makeatletter
\def\@bibdata#1{\write\@auxout{\string\bibdata{#1}}}
\def\@bibstyle#1{\write\@auxout{\string\bibstyle{#1}}}
\makeatother
...
% Colors [previous color definitions]
...
% Beamer theme settings [previous settings]
...
\begin{comment}
\begin{frame}
       \frametitle{LPM vs Probit: Sign Reversal Example}
% Use scope to apply different scaling to x and y
\begin{tikzpicture}
       \begin{scope}[x=1cm,y=3cm]  % Elongated vertical scale
           % Style definitions
           \tikzset{
               untreated point/.style={circle, fill=blue, opacity=0.7, inner sep=2pt},
               treated point/.style={circle, fill=red, opacity=0.7, inner sep=2pt},
           }
           
           % Axes
           \draw[axis line/.style={thick, ->}] (-3,0) -- (11,0) node[right] {$R$};
           \draw[axis line/.style={thick, ->}] (0,0) -- (0,1.2) node[above] {$D$};
           ...
           % Data points - Untreated (blue)
           \node[untreated point] at (-1.8,0) {};  % Point 1
           \node[untreated point] at (-0.9,1) {};  % Point 2
           \node[untreated point] at (-0.92,1) {}; % Point 3
           
           % Data points - Treated (red)
           \node[treated point] at (-2.1,0) {};    % Point 4
           \node[treated point] at (-1.92,1) {};   % Point 5
           \node[treated point] at (10,1) {};      % Point 6
           
           % LPM line with slope -0.15
           \draw[black, thick, dashed] (-2.5,0.8) -- (10,-0.7);
           
           % Legend
           \node[anchor=south west, fill=white, draw=black, inner sep=2pt]
               at (5,0.3) {
               \begin{tabular}{ll}
                   \textcolor{blue}{$\bullet$} & Untreated \\
                   \textcolor{red}{$\bullet$} & Treated \\
                   \textcolor{black}{\rule{1cm}{1pt}} & LPM Fit
               \end{tabular}
               };
       \end{scope}
   \end{tikzpicture}
